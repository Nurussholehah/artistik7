function cek1(){
	satu=(document.getElementById("b1").value);
	
	if (satu==100){
	document.getElementById("b1").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b1").style="border: 3px solid red; width:50px;"
	}
}

function cek2(){
	satu=(document.getElementById("b2").value);
	
	if (satu==900){
	document.getElementById("b2").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b2").style="border: 3px solid red; width:50px;"
	}
}

function cek3(){
	satu=(document.getElementById("b3").value);
	
	if (satu==10% || satu=="10%"){
	document.getElementById("b3").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b3").style="border: 3px solid red; width:50px;"
	}
}

function cek4(){
	satu=(document.getElementById("b4").value);
	
	if (satu==900){
	document.getElementById("b4").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b4").style="border: 3px solid red; width:50px;"
	}
}

function cek5(){
	satu=(document.getElementById("b5").value);
	
	if (satu==100){
	document.getElementById("b5").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b5").style="border: 3px solid red; width:50px;"
	}
}

function cek6(){
	satu=(document.getElementById("b6").value);
	
	if (satu==90.000 || satu=="90000"){
	document.getElementById("b6").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b6").style="border: 3px solid red; width:50px;"
	}
}

function cek7(){
	satu=(document.getElementById("b7").value);
	
	if (satu==90.000 || satu=="90000"){
	document.getElementById("b7").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b7").style="border: 3px solid red; width:50px;"
	}
}

function cek8(){
	satu=(document.getElementById("b8").value);
	
	if (satu==10){
	document.getElementById("b8").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b8").style="border: 3px solid red; width:50px;"
	}
}

function cek9(){
	satu=(document.getElementById("b9").value);
	
	if (satu==90.000 || satu=="90000"){
	document.getElementById("b9").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b9").style="border: 3px solid red; width:50px;"
	}
}

function cek10(){
	satu=(document.getElementById("b10").value);
	
	if (satu==90){
	document.getElementById("b10").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b10").style="border: 3px solid red; width:50px;"
	}
}

function cek11(){
	satu=(document.getElementById("b11").value);
	
	if (satu==100.000 || satu=="100000"){
	document.getElementById("b11").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b11").style="border: 3px solid red; width:50px;"
	}
}

function cek12(){
	satu=(document.getElementById("b12").value);
	
	if (satu==100.000 || satu=="100000"){
	document.getElementById("b12").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b12").style="border: 3px solid red; width:50px;"
	}
}

function cek13(){
	satu=(document.getElementById("b13").value);
	
	if (satu==100){
	document.getElementById("b13").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b13").style="border: 3px solid red; width:50px;"
	}
}


function cek14(){
	satu=(document.getElementById("b14").value);
	
	if (satu==1.000 || satu=="1000"){
	document.getElementById("b14").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b14").style="border: 3px solid red; width:50px;"
	}
}

function cek15(){
	satu=(document.getElementById("b15").value);
	
	if (satu==1.000 || satu=="Rp1000" || satu=="Rp1000,00"){
	document.getElementById("b15").style="border: 3px solid green; width:50px;"
	}
	
	else{
	document.getElementById("b15").style="border: 3px solid red; width:50px;"
	}
}